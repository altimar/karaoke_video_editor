"""Export facebook/wav2vec2-large-960h-lv60-self → single-file ONNX (fp32),
then fp16-convert it. Input: input_values [B,T] float32 → logits [B,F,32]
(the same 32-token vocab as the app's ctc.ts).

Usage (venv with torch, transformers, onnx, onnxruntime, onnxscript,
onnxconverter-common):
    python eval/export-large-align.py
Outputs (into kfn/, gitignored): wav2vec2-large-lv60.onnx (+fp16).
Uploaded to HF: Project42/wav2vec2-large-lv60-align.
"""
import numpy as np
import onnx
...
OUT = 'kfn/wav2vec2-large-lv60.onnx'
OUT16 = 'kfn/wav2vec2-large-lv60-fp16.onnx'
...
# The dynamo exporter writes external data — embed into one file (<2 GB).
