...
- **Аудио** (`AudioTrack`, `type: 'audio'`): ссылка на источник (`audioFileName`) и огибающая громкости (`volumeAutomation`). Сырые байты аудио хранятся ВНЕ проекта (в audioEngine/controls); дорожка хранит только имя + automation. Аудиодорожки не рендерятся в кадр видео.
