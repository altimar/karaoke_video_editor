...
- **Видео (MP4)**: в модели — только имя файла (`bgVideoFileName`), сырые байты живут ВНЕ проекта, в `lib/backgroundVideo.ts` (по образцу аудио-байтов в `audioLoader`). Модуль также владеет скрытым muted `<video>`-элементом — источником живых кадров для превью.
