...
- **Аудио** (`AudioTrack`, `type: 'audio'`): фиксированная `role` (`'original' | 'minus' | 'back'`), ссылка на источник (`audioFileName`, пусто = нет аудио) и огибающая громкости (`volumeAutomation`). Сырые байты аудио хранятся ВНЕ проекта (в `audioLoader`); дорожка хранит только роль + имя файла + automation. Аудиодорожки не рендерятся в кадр видео. Проект всегда содержит все три роли; «оригинал» — для записи таймингов, «минус»+«бэк» — микшируются в экспорт видео.
...
src/
  ...
  lib/
    ...
    audioEngine.ts         — мульти-голосовое воспроизведение по ролям (оригинал/минус/бэк), декодирование
